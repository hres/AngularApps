...
import {
  EQUALS,
  INGREDIENT_FORMULATION_ERROR_PREFIX,
  MEASURE,
  MEDICINAL,
  NON_MEDICINAL,
  NOT_LESS,
  NOT_MORE,
  OPTIONAL_FIELDS_INGREDIENT_FORMULATION,
  PRESENTATION,
  RANGE,
  UNITS_OTHER,
  UNIT_MEASURE_OTHER,
...
export class FormulationItemComponent extends BaseComponent {
  ...
  private async _save(index: number) {
    ...
    if (this.cRRow.valid) {
      const heading = await this._formulationService.getHeading(
        index,
        this.cRRow
      ); // Await here
  ...
  public async deleteFormulationRecord(index: number) {
    ...
    const heading = await this._formulationService.getHeading(
      index,
      this.cRRow
    ); // Set heading here for when the record isn't saved yet
    this.cRRow.get('heading').setValue(heading);
    this.deleteRecord.emit({
      index: index,
      heading: this.cRRow.get('heading').value,
    });
  }
  ...
  private _handleRecordDeletion() {
    this._errNotifService.updateErrorSummary(
      INGREDIENT_FORMULATION_ERROR_PREFIX + this.cRRow.get('id').value,
      null
    );
    this.cRRow.markAsPristine();
  }
