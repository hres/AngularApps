...
export abstract class BaseListComponent<T extends OutputRecord>
  extends BaseComponent
  implements IBaseList<T>, AfterViewInit
{
  @Input() recordList: T[];
  ...
  private async _init(recordData: T[]): Promise<void> {
    ...
    if (recordData && recordData.length !== 0) {
      ...
      for (const [index, record] of recordData.entries()) {
        ...
        this._patchLastSavedStateValue(
          group.controls['lastSavedState'],
          record,
        );
  ...
  protected abstract _patchLastSavedStateValue(
    lastSavedStateFormControl,
    outputModel,
  );
  ...
  saveRecord(event: any): void {
    ...
    const id = group.get('isNew').value
      ? this.listService.getNextId()
      : group.get('id').value;
    group.patchValue({
      id: id,
      isNew: false,
  ...
  protected _expandNextInvalidRecord() {
    ...
    for (let index = 0; index < this.recordFormArray.controls.length; index++) {
      const group: RecordFormGroup = this.recordFormArray.controls[
        index
      ] as RecordFormGroup;
      if (group.invalid) {
  ...
  onDiscardConfirmed(index: number) {
    if (this.componentName === 'address') {
      this._recordDiscardService.confirmDiscardAddress(index);
    } else if (this.componentName === 'contact') {
      this._recordDiscardService.confirmDiscardContact(index);
    } else {
  ...
  onDeleteConfrmed(index: number) {
    if (this.componentName === 'address') {
      this._recordDeleteService.confirmDeleteAddress(index);
    } else if (this.componentName === 'contact') {
      this._recordDeleteService.confirmDeleteContact(index);
    } else {
  ...
  onDeleteHandled(event: any) {
    if (event) {
      for (
        let index = 0;
        index < this.recordFormArray.controls.length;
        index++
      ) {
        const group: RecordFormGroup = this.recordFormArray.controls[
          index
  ...
  handleRowClick(event: any): void {
    ...
    if (this.recordFormGroup.pristine) {
      this.recordFormArray.controls.forEach(
